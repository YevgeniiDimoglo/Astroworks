...
{
    vec3 flow = vec3(Animation.r * timerConstants.g * 0.3, Animation.g * timerConstants.g * 0.4, Animation.b * timerConstants.g * 0.5);
	float noise = Turbulence(position * NoiseFrequency + flow, 0.1, 1.5, 0.03) * NoiseAmplitude;
...
vec4 March(vec3 rayOrigin, vec3 rayStep)
{
	...
	return mix(Shade(displacement), vec4(0.f), float(distance >= 0.5));
}
...
void main()
{
	vec2 p = (inUV.xy) * 2.0 - 1.0;
	float zoom = 5.0;
	// // camera
	vec3 ro = zoom * normalize(vec3(1.f,1.f,1.f));;
	...
	vec4 col = vec4(0.f);
	col = March(ro, rd);
	col = vec4(col.rgb, (col.r + col.g + col.b) / 3.0 * 1.1 - 0.1);
    outColor = col;
}
